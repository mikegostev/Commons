...

{
 ...
 protected Constructor getLogConstructor() throws LogConfigurationException
 {
  ...
  try
  {
   ...
   if(!logInterface.isAssignableFrom(logClass))
   {
    ...
    for(int i = 0; i < interfaces.length; i++)
    {
     ...
     {
      throw new LogConfigurationException("Invalid class loader hierarchy.  " + "You have more than one version of '"
        + LOG_INTERFACE + "' visible, which is " + "not allowed.");
     }
    }
    throw new LogConfigurationException("Class " + logClassName + " does not implement '" + LOG_INTERFACE + "'.");
   }
  }
  catch(Throwable t)
  {
  ...
  {
   throw new LogConfigurationException("No suitable Log constructor " + logConstructorSignature + " for "
     + logClassName, t);
  }
 }
