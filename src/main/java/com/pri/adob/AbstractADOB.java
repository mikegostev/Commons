...
public abstract class AbstractADOB implements MutableADOB
{
 ...
 /*
 private static String extractParam( String str, String param )
 {
  if( str == null )
   return null;
  ...
  try
  {
   ...
   do
   {
    ...
    if(epos == -1)
    {
     part = str.substring(pos).trim().toLowerCase();
     epos = str.length();
    }
    else
     part = str.substring(pos, epos).trim().toLowerCase();
    ...
    part = str.substring(pos + 1, epos).trim();
