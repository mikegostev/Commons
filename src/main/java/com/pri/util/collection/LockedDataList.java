...
/**
 * <p>
 * Completely synchronized {@link DataList} implementation. The
 * idea is the same as in {@link DefaultDataList} but this implementation can use an
 * external {@link java.util.concurrent.locks.Lock} object for synchronization
 * purposes. The lock to use must be given in the constructor. If null is given,
 * then a new instance of {@link java.util.concurrent.locks.ReentrantLock} will
 * be used. Use {@link #getLock} method to retrieve
 * the lock.
 * 
 * @author vaz
 */
public class LockedDataList<T> implements DataList<T> {
	...
	public void add(int index, T element) {
		...
		try {
			...
			model
					.fireIntervalAdded((T[]) new Object[] { element}, index,
							index);
		} finally {
			lock.unlock();
		}
	...
	public boolean addAll(Collection<? extends T> c) {
		...
		try {
			...
			int to = size();
			if (result)
				model.fireIntervalAdded(c.toArray((T[]) new Object[c.size()]),
						from, to);
			return result;
	...
	public void clear() {
		...
		try {
			...
			delegate.clear();
			if (last > 0) model.fireIntervalRemoved(oldData, 0, last - 1);
		} finally {
			lock.unlock();
		}
	}
	...
	public boolean remove(Object o) {
		...
		try {
			...
			if (result) model.fireIntervalRemoved(oldData, index, index);
			return result;
		} finally {
			lock.unlock();
		}
	...
	public T set(int index, T element) {
		...
		try {
			...
			model.fireIntervalChanged((T[]) new Object[] { element},
					(T[]) new Object[] { t}, index, index);
			return t;
		} finally {
			lock.unlock();
		}
	...
	public T getByID(@SuppressWarnings("unused")
	int id) {
		throw new UnsupportedOperationException(
				"I have no idea how to extract IDs from these elements.");
	}
	private class Model extends AbstractListModel implements ListDataModel<T> {
		...
		public void fireIntervalRemoved(T[] oldValues, int from, int to) {
			...
			try {
				...
				for (int i = listeners.length - 2; i >= 0; i -= 2) {
					if (listeners[i] == ListListener.class) {
						((ListListener<T>) listeners[i + 1]).intervalRemoved(
								oldValues, from, to);
					}
				}
		...
		@SuppressWarnings("unchecked")
		public void fireIntervalChanged(T[] newValues, T[] oldValues, int from,
				int to) {
			lock.lock();
			try {
				Object[] listeners = listenerList.getListenerList();
				...
				for (int i = listeners.length - 2; i >= 0; i -= 2) {
					if (listeners[i] == ListListener.class) {
						((ListListener<T>) listeners[i + 1]).intervalChanged(
								newValues, oldValues, from, to);
					}
				}
