 ...
 * @author vaz
 *         <p>
 *         Completely synchronized List implementation. Use synchronized(list)
 *         block when iterating over this list. This way you won't get
 *         ConcurrentModificationException: <code>
 *         ... list = new ArrayListModel () ;
 *         ...
 *         synchronized (list){
 *           for (Object o:list){
 *           // do you dirty work here
 *           }
 *         }
 *         </code>
 */
public class DefaultDataList<T> implements DataList<T> {
	...
	private class Model extends AbstractListModel implements ListDataModel<T> {
		...
		@SuppressWarnings("unchecked")
		synchronized public void fireIntervalAdded(T[] newValues, int from,
				int to) {
		...
		@SuppressWarnings("unchecked")
		synchronized public void fireIntervalRemoved(T[] oldValues, int from,
				int to) {
			Object[] listeners = listenerList.getListenerList();
		...
		@SuppressWarnings("unchecked")
		synchronized public void fireIntervalChanged(T[] newValues,
				T[] oldValues, int from, int to) {
			Object[] listeners = listenerList.getListenerList();
