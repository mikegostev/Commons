...
/**
 * <p>A usefull implementation of an event-like queue. Supports adding and removing
 * elements while iterating over the queue. The implementation garantees that
 * everytime you ask for an element from iterator it will use the most recent
 * information about elements. As a sideeffect the queue doesn't allow null elements since 
 * iteration cycle looks something like this: </p>
 * <code>
 * ...
 * Object next = null ;
 * do {
...
public class ConcurrentLinkedQueue<E> implements Iterable<E> {
	...
	public synchronized boolean contains(E obj) {
		...
		while (el != marker) {
			if (el.obj.equals(obj)) return true;
  ...
  public E next() {
			synchronized (ConcurrentLinkedQueue.this) {
				...
				if (ptr.count == 0 && ptr.prev.side == ptr) {
					...
					if (ptr.side != null) ptr.side.prev = ptr.prev;
				}
