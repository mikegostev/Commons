...
public class LongHashMap<V> extends LongAbstractMap<V> implements LongMap<V>, Cloneable, Serializable {
     ...
     * contains no mapping for this key. A return value of <tt>null</tt> does not <i>necessarily</i> indicate that the
     * map contains no mapping for the key; it is also possible that the map explicitly maps the key to <tt>null</tt>.
     ...
     * <tt>null</tt> return can also indicate that the HashMap previously associated <tt>null</tt> with the specified
     * key.
     */
    @Override
    @SuppressWarnings("unchecked")
    ...
    void putAllForCreate(LongMap<? extends V> m) {
        for (Iterator<? extends LongMap.Entry<? extends V>> i = m.entrySet().iterator(); i.hasNext(); ) {
            LongMap.Entry<? extends V> e = i.next();
   ...
   */
    ...
    Entry<V> removeMapping(Object o) {
        ...
        int i = indexFor(longHash(entry.getKey()), table.length);
        Entry<V> prev = table[i];
        Entry<V> e = prev;
    ...
    static class Entry<T> implements LongMap.Entry<T> {
        ...
        @Override
        public int hashCode() {
            return (int) (key & 0xFFFFFFFF) ^ (int) (key >> 32) ^ (value == null ? 0 : value.hashCode());
        }
         ...
         */
        void recordAccess(@SuppressWarnings("unused") LongHashMap<T> m) {
        }

        /**
         * This method is invoked whenever the entry is removed from the table.
         */
        void recordRemoval(@SuppressWarnings("unused") LongHashMap<T> m) {
        }
    }
    ...
    private class EntryIterator extends HashIterator<LongMap.Entry<V>> {
     ...
     * to the map are reflected in the collection, and vice-versa.  The collection supports element removal, which
