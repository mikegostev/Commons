...
public class SessionPoolLocalMap<CS extends ClientSession> implements SessionPool<CS>
{
 ...
 public void removeExpiredSessions()
 {
  ...
  for(int i = 0, l = kArr.length; i < l; i++)
  {
   ...
   if(log.isTraceEnabled())
    log.trace("Checking client " + cl.getUserName() + "(" + cl.getUserID() + ")@" + cl.getAddr() + " lastUpdTime="
      + (new java.util.Date(cl.lastUpdateTimestamp()).toString()));
   ...
   Interval updateTimeIval, Interval expireTimeIval)
 {
  ...
  for(CS dt : pool.values())
  {
   ...
   if(startTimeIval != null && !startTimeIval.contains(CalendarUtils.epoch70ToEpoch2kSec(dt.creationTimestamp())))
    continue;

   if(updateTimeIval != null && !updateTimeIval.contains(CalendarUtils.epoch70ToEpoch2kSec(dt.creationTimestamp())))
    continue;

   if(expireTimeIval != null && !expireTimeIval.contains(CalendarUtils.epoch70ToEpoch2kSec(dt.creationTimestamp())))
    continue;
