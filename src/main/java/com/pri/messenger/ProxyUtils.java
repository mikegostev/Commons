...
// }
 ...
 {
  if( !(mb instanceof ADOBMessageBody) 
    || ! EXCEPTION_CONTENT_TYPE.equalsIgnoreCase(((ADOBMessageBody)mb).getContentType()) )
   return;
   ...
   while( true )
   {
    ...
    try
    {
     ...
     if( ErrorInfo.class.isAssignableFrom( extClass ) )
     {
      try
      {
       t = (Throwable)extClass
        .getConstructor( new Class[]{String.class,int.class,Object.class})
     ...
     if( t == null )
      t = (Throwable)extClass
       .getConstructor( new Class[]{String.class}).newInstance(ei.getMessage());
    }
    catch (ClassNotFoundException e) 
    {
     t = new Exception("Unavailable class ("+ei.getExceptionClass()+") exception: "+ei.getMessage());
    }
    catch(Exception e)
    {
     t = new Exception("Unavailable class ("+ei.getExceptionClass()+") exception (init error: '"+e+"'): "+ei.getMessage());
    }
