...
*/
 ...
 public Message syncReceive(Address address, long timeout)
   throws SyncReceiveTimeoutException
 {
  RecipientMonitor monitor = resolver.addSyncRecipient(address);
  synchronized (monitor)
  {
 ...
 private void generalSend(Message message, ProgressListener pLsnr, ADOBFactory af) throws RecipientNotFoundException
 {
...
{
 public RecipientMonitor addRecipient(MessageRecipient recipient,
   Address address);
...
class HashMonitorAddressResolver implements MonitorAddressResolver
{
 ...
 private Map<String,MonitorsSet> addressToRecipientMap 
 = Collections.synchronizedMap(new TreeMap<String,MonitorsSet>()); //new Hashtable();
