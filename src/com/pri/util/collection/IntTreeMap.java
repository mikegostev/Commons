...
public class IntTreeMap<V> extends IntAbstractMap<V> implements Cloneable, SortedIntMap<V>,
...
*/
  ...
  */
 public IntTreeMap(Map<Integer, ? extends V> m)
 {
  putAll(m);
 }

 public IntTreeMap(IntMap< ? extends V> m)
 {
  putAll(m);
 }
  ...
  */
 public IntTreeMap(SortedMap<Integer, ? extends V> m)
 {
  try
  {
  ...
  *                  <tt>null</tt> keys.
  */
 private Entry<V> getEntry(int k)
 {
  Entry<V> p = root;
...
//   int cmp = k - p.key;
  ...
  */
 private Entry<V> getCeilEntry(int key)
 {
  Entry<V> p = root;
  if (p == null)
   return null;
...
//   int cmp = key - p.key;
  ...
  */
 private Entry<V> getPrecedingEntry(int key)
 {
  Entry<V> p = root;
  if (p == null)
   return null;
...
//   int cmp = key - t.key;
 ...
 public Object clone()
 {
  IntTreeMap<V> clone = null;
  try
  {
   clone = (IntTreeMap<V>) super.clone();
  }
  ...
  */
 private transient volatile Set<IntMap.Entry<V>> entrySet = null;
 ...
 public Set<IntMap.Entry<V>> entrySet()
 {
  if (entrySet == null)
  {
   entrySet = new AbstractSet<IntMap.Entry<V>>()
   {
  ...
  */
 public SortedIntMap<V> subMap(int fromKey, int toKey)
 {
  ...
  */
 public SortedIntMap<V> headMap(int toKey)
 {
  ...
  *               tolerate <tt>null</tt> keys.
  */
 public SortedIntMap<V> tailMap(int fromKey)
 {
  return new SubMap(fromKey, false);
 }

 private class SubMap extends IntAbstractMap<V> implements IntMap<V>,SortedIntMap<V>,
   java.io.Serializable
 {
  ...
  public int firstKey()
  {
   IntTreeMap.Entry<V> e = fromStart ? firstEntry() : getCeilEntry(fromKey);
   int first = key(e);
  ...
  public int lastKey()
  {
   IntTreeMap.Entry<V> e = toEnd ? lastEntry() : getPrecedingEntry(toKey);
   int last = key(e);
  ...
  private transient Set<IntMap.Entry<V>> entrySet = new EntrySetView();
  ...
  private class EntrySetView extends AbstractSet<IntMap.Entry<V>>
  {
   ...
   public int size()
   {
    ...
    {
     size = 0;
     sizeModCount = IntTreeMap.this.modCount;
     Iterator i = iterator();
     while (i.hasNext())
     {
      size++;
   ...
   public boolean remove(Object o)
   {
    ...
    if (!inRange(key))
     return false;
    IntTreeMap.Entry<V> node = getEntry(key);
   ...
   public Iterator<IntMap.Entry<V>> iterator()
   {
    return new SubMapEntryIterator((fromStart ? firstEntry()
      : getCeilEntry(fromKey)), (toEnd ? null : getCeilEntry(toKey)));
   }
  }
 ...
 private class EntryIterator extends PrivateEntryIterator<IntMap.Entry<V>>
 {
 ...
 private class KeyIterator extends PrivateEntryIterator<Integer>
 {
  public Integer next()
  {
 ...
 private class SubMapEntryIterator extends
   PrivateEntryIterator<IntMap.Entry<V>>
 {
 ...
 static class Entry<VE> implements IntMap.Entry<VE>,Serializable
 {
  int       key;
  VE        value;
  Entry<VE> left  = null;
   ...
   */
  Entry(int key, VE value, Entry<VE> parent)
  {
   this.key = key;
   this.value = value;
   this.parent = parent;
 ...
 void addAllForTreeSet(SortedSet<IntMap.Entry<V>> set, V defaultVal)
 {
  try
  {
   ...
   throws java.io.IOException, ClassNotFoundException
 {
  ...
  if (it != null)
  {
   if (defaultVal == null)
   {
    IntMap.Entry<V> entry = (IntMap.Entry<V>) it.next();
    key = entry.getKey();
  ...
  if (mid < hi)
  {
   Entry<V> right = buildFromSorted(level + 1, mid + 1, hi, redLevel, it, str,
     defaultVal);
   middle.right = right;
   right.parent = middle;
  }
