...
//   conn.setRequestProperty("Pragma","no-cache");
 ...
 public boolean setConnected(boolean state) throws NetworkException
 {
  if (state)
  {
   ...
   try
   {
    ...
    NetworkInputRequest netreq = NetworkMessengerProtocol.parseResponse(conn.getContentType(), conn.getInputStream(),
 ...
 public void syncSend(Message message, ADOBFactory adobFact, final ProgressListener pLsnr) throws RecipientNotFoundException, NetworkException
 {
  if( currentState != State.CONNECTED )
   throw new NetworkException("Network messenger not connected");
  ...
  try
  {
   ...
   if( pLsnr != null )
   {
    ...
    conIS=new ListenedInputStream(conn.getInputStream(),
      new PumpListener()
      {
       public void dataPumped(int k)
       {
   ...
   NetworkInputRequest netreq = NetworkMessengerProtocol.parseResponse(
      conn.getContentType(),conIS,new HeadersIterator(conn.getHeaderFields()), adobFact);
...
// System.out.println("RespCode: "+respCode);
     ...
     if( respCode != HttpURLConnection.HTTP_OK )
     {
      ...
      if( respCode == HttpURLConnection.HTTP_GATEWAY_TIMEOUT || respCode == HttpURLConnection.HTTP_CLIENT_TIMEOUT )
       connectTries--;
      else
       Thread.sleep(3*1000);
     ...
     NetworkInputRequest netreq = NetworkMessengerProtocol.parseResponse(
       conn.getContentType(),conn.getInputStream(),new HeadersIterator(conn.getHeaderFields()), null );
